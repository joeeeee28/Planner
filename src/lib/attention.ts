// Growth OS V4 Slice 3 — attention engine.
...
// Rules are intentionally conservative and ordered by severity.
...
export function attentionItems(data: AppData, opts: AttentionOptions = {}): AttentionItem[] {
  ...
  // 1) Goals — health-driven (overdue > at risk > needs attention)
  ...
  // 2) High-priority open tasks past their planned day
  ...
  // 3) Budgets — near limit or over (existing semantics kept)
  ...
  // 4) Savings goals — target date near or passed, or clearly behind pace
  ...
  // 5) In-progress goals whose linked savings pace is far behind (only when
  //    there is real contribution history to compare) — covered by health.
  //    Recurring schedules due soon are informational, not attention.
